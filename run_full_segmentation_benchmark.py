...
def main():
    ...
    if not args.skip_yolo:
        cmd = f'{python_exe} segmentation/training/train_yolo_mlflow.py --model yolo11n-seg.pt --epochs {args.epochs} --batch_size 8 --workers 1 --experiment_name "MFF_Benchmark_YOLO11" --save_dir "data/models/segmentation/yolo/yolo11n"'
        run_command(cmd, "Train YOLO11-Nano")
        
        # Rename best.pt to best_yolo11n.pt for benchmark script
        src = "data/models/segmentation/yolo/yolo11n/best.pt"
        dst = "data/models/segmentation/yolo/best_yolo11n.pt"
        if os.path.exists(src):
            import shutil
            shutil.copy(src, dst)
            print(f"Copied {src} to {dst}")
    ...
    if not args.skip_yolo:
        cmd = f'{python_exe} segmentation/training/train_yolo_mlflow.py --model yolo11s-seg.pt --epochs {args.epochs} --batch_size 8 --workers 1 --experiment_name "MFF_Benchmark_YOLO11" --save_dir "data/models/segmentation/yolo/yolo11s"'
        run_command(cmd, "Train YOLO11-Small")
        
        # Rename best.pt to best_yolo11s.pt for benchmark script
        src = "data/models/segmentation/yolo/yolo11s/best.pt"
        dst = "data/models/segmentation/yolo/best_yolo11s.pt"
        if os.path.exists(src):
            import shutil
            shutil.copy(src, dst)
            print(f"Copied {src} to {dst}")
    ...
    # 6. Run Benchmark
    cmd = f'{python_exe} segmentation/inference/benchmark_segmentation.py --maskrcnn_weights "data/models/segmentation/weight/best_model.pth" --yolo_nano_weights "data/models/segmentation/yolo/best_n.pt" --yolo_small_weights "data/models/segmentation/yolo/best_s.pt" --yolo11_nano_weights "data/models/segmentation/yolo/best_yolo11n.pt" --yolo11_small_weights "data/models/segmentation/yolo/best_yolo11s.pt" --output_dir "data/outputs/exps/benchmark_final"'
