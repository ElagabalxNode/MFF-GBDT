...
# Go up 3 levels: training/ -> gbdt/ -> training_workspace/ -> project_root/
...
def parse_args():
    ...
    # Data paths
    parser.add_argument("--train_path", type=str, default="training_workspace/data/processed/csvData/processed_fixed/train_fixed.csv", help="Path to training data")
    parser.add_argument("--val_path", type=str, default="training_workspace/data/processed/csvData/processed_fixed/val_fixed.csv", help="Path to validation data")
    parser.add_argument("--test_path", type=str, default="training_workspace/data/processed/csvData/processed_fixed/test_fixed.csv", help="Path to test data")
    ...
    parser.add_argument("--exclude_auto_features", action="store_true", help="Exclude auto-features (columns starting with 'auto_')")
    
    return parser.parse_args()

def get_xy(df, exclude_auto=False):
    ...
    if exclude_auto:
        # Filter out columns starting with 'auto_'
        manual_cols = [c for c in X.columns if not c.startswith('auto_')]
        X = X[manual_cols]
        print(f"Filtered out auto-features. Remaining features: {len(X.columns)}")
...
def train_and_evaluate(args):
    ...
    with mlflow.start_run(run_name=args.run_name) as run:
        ...
        x_train, y_train = get_xy(df_train, args.exclude_auto_features)
        x_val, y_val = get_xy(df_val, args.exclude_auto_features)
        x_test, y_test = get_xy(df_test, args.exclude_auto_features)
